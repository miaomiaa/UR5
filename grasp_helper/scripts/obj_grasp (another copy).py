...
class MoveRobot():
    ...
    def goSP(self):
        ...
        self.arm_group.set_joint_value_target([1.57, 1.0, 2.09, 0, -1.5, -1.57 ])

        ###1.582, -0.0527,1.713,-0.15092505292892455, -0.15092505292892455, -1.57
        self.arm_group.go()
    def goSP2(self):

        self.arm_group.set_joint_value_target([2.91, -0.244,1.849,0, 0.98, -1.4 ])


        self.arm_group.go()
    def goSP3(self):

        self.arm_group.set_joint_value_target([1.545, -1.01,-1.74,-0.043, 1.158, 1.57  ])


        self.arm_group.go()
    def goSP4(self):

        self.arm_group.set_joint_value_target([3, -0.0527,2.2,-0.15092505292892455, -0.15092505292892455, -1.57   ])
    ...
    def grasp_obj(self):
        ...
        y0=self.Obj_pose.pose.position.y
        if 0<=x0<=0.04:
           x1=x0+0.035
        elif 0.05<x0<=0.08:
           x1=x0+0.04
        ...
        self.Obj_pose.pose.position.x = x2
        self.Obj_pose.pose.position.y = y3
        self.Obj_pose.pose.position.z = z2+0.06
        self.arm_group.set_pose_target(self.Obj_pose.pose)
        self.plan_cartesian_path(self.Obj_pose.pose)           

         
        self.Obj_pose.pose.position.y = y2+0.2
